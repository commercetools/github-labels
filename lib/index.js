...
async function labelControl() {
  ...
  const illegalLabels = checkForSameLabelName(
    localConfig,
    formattedPublishedLabels
  );
  if (illegalLabels) {
    throw new Error(
      'The config has labels with the same name. The Label names have to be unique.'
    );
  }
...
function checkForSameLabelName(localConfig, publishedConfig) {
  const illegalLabels = localConfig.map(localLabel => {
    const illegalname = publishedConfig.find(publicLabel => {
      return localLabel.name === publicLabel.name ? publicLabel : undefined;
    });
    return illegalname;
  });
  return illegalLabels;
}
